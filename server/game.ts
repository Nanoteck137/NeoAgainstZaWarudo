  ...
  playerPlay(io: Server, player: Player, handIndex: number) {
    ...
    if (hand && this.judge !== player.id) {
      ...
      if (playerBoard!.length === maxPlayableCards) {
        ...
        io.to(player.id).emit("game:roundUpdate", {
          cardToBePlayed: remaining,
        });
      }

      io.to(player.id).emit(
        "game:updateHand",
        this.getHandFromPlayerId(player.id)
      );
    }
    ...
    if (room) {
      for (let player of room.playerIds) {
        if (player !== this.judge) {
          ...
          io.to(player).emit(
            "game:updateHand",
            this.getHandFromPlayerId(player)
          );
        }
      }
    }
