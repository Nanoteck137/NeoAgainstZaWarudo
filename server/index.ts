...
import { games, GameSettings } from "./game";
import { getCardPacks, readCardList } from "./card";
...
function joinRoom(socket: Socket, player: Player, roomId: string) {
  ...
  if (newRoom) {
    ...
    let playerObjects = newRoom!
      .getPlayerObjects()
      .map((p) => p.toClientObject());
    socket.emit("client:joinedRoom", newRoom.toClientObject(), playerObjects);
...
io.on("connection", (socket: Socket) => {
  ...
  socket.on("client:login", (data, callback) => {
    ...
    socket.on("room:getGameSettings", (callback) => {
      let player = getPlayerBySocket(socket);
      if (player && player.currentRoom) {
        let room = getRoomById(player.currentRoom);
        if (room) {
          let settings = room.getGameSettings();
          let packs = getCardPacks();
          callback(settings, packs);
        }
      }
    });

    socket.on("room:setGameSettings", (newSettings: GameSettings) => {
      let player = getPlayerBySocket(socket);
      if (player && player.currentRoom) {
        let room = getRoomById(player.currentRoom);
        if (room) {
          room.setGameSettings(newSettings);
        }
      }
    });
